...
def save_model2(args, model, clientsSavedParams, current_epoch):
    out = os.path.join(args.model_path, "checkpoint_{}.tar".format(current_epoch))
    state = {'net': model.state_dict(), 'epoch': current_epoch}
    torch.save(state, out)
    with open("./datasets/cifar10/clientsSavedParams", "wb") as f:
        pickle.dump(clientsSavedParams, f) 
...
def createNoIIDClientDataset():
    ...
    for c in range(args.n_clients):
        if c==0:
            newSample = torch.tensor(Sample[clientDataIndex[c]])
            newLabel= torch.tensor(Label[clientDataIndex[c]])
        else:
            newSample = torch.cat([newSample, Sample[clientDataIndex[c]].clone()], dim=0)
            newLabel = torch.cat([newLabel, Label[clientDataIndex[c]].clone()], dim=0)

    with open("./datasets/cifar10/SampleNoiid", "wb") as f:
        pickle.dump(newSample, f)  
    with open("./datasets/cifar10/LabelNoiid", "wb") as f:
        pickle.dump(newLabel, f) 


def createNoiidClientDataset():
    clientDataIndex={}
    with open("./datasets/cifar10/SampleNoiid", "rb") as f:
        Sample=pickle.load(f) 
...
class Aggregator:
    def __init__(self, device):
        ...
        checkpoint = torch.load("save/Img-10-test/checkpoint_68.tar",map_location=torch.device('cpu'))['net']
        # checkpoint = torch.load('save/Img-10-FL/checkpoint_634.tar',map_location=torch.device('cpu'))['net']
...
def getRegularizationLoss(model, agg):
    flag=0
    for name, param in model.named_parameters ():
        if param.requires_grad:
            update = (agg[name].data - param.data).clone()
            if flag==0:
                normSum = torch.sum(torch.pow(update, exponent=2))
                flag=1
            else:
                normSum += torch.sum(torch.pow(update, exponent=2))
    return normSum
...
def train(agg, round):
    ...
    for batch_idx, (x_list) in enumerate(dataLoader):
        ...
        lossPerUser=0
        true_epoch=0
        prev_model_path = os.path.join("save/{}_prevModel/".format(args.dataset), "client{}.tar".format(c))
        ...
        for i in range(args.local_epoch):
            for j in range(i+1, args.local_epoch):
                ...
                x_j.requires_grad = False
                z_i, z_j, c_i, c_j = model(x_i, x_j)
                loss_instance = criterion_instance(z_i, z_j)
                loss_cluster = criterion_cluster(c_i, c_j)

                with torch.no_grad():
                    global_z_i, _, global_c_i, _ = agg.model(x_i, x_j)
                poszi = cos(z_i, global_z_i)
                logits_zi = poszi.reshape(-1,1)
                posci = cos(c_i, global_c_i)
                logits_ci = posci.reshape(-1,1)

                if round>0:
                    prev_checkpoint = torch.load(prev_model_path, map_location=device)['net']
                    modelcopy.load_state_dict(prev_checkpoint, strict=False)
                    with torch.no_grad():
                        prev_z_i, _, prev_c_i, _ = modelcopy(x_i, x_j)
                    poszi_prev = cos(z_i, prev_z_i)
                    logits_zi = torch.cat((logits_zi, poszi_prev.reshape(-1,1)), dim=1)
                    posci_prev = cos(c_i, prev_c_i)
                    logits_ci = torch.cat((logits_ci, posci_prev.reshape(-1,1)), dim=1)
                
                logits_zi /= args.model_temperature
                logits_ci /= args.model_temperature
                labels = torch.zeros(x_i.size(0)).to(device).long()
                loss_model = args.miu_cm * (criterion_model(logits_zi, labels) + criterion_model(logits_ci, labels))
        ...
        torch.save(state, prev_model_path)

        normSum = getModelUpdate(gradDict, agg.model.state_dict(), model, clientsSavedParams[c]) # model - modelcopy   
    ...
    if args.clip_bound > 1.6:
        args.clip_bound=args.clip_bound - 0.3
    else:
        args.clip_bound = sum(normlist)/len(normlist)
        if args.clip_bound > 1.5:
            args.clip_bound = 1.5
...
if __name__ == "__main__":
    ...
    config = yaml_config_hook("config/config_DP_FL_Cifar_fix_noiid.yaml")
    ...
    with open("./datasets/cifar10/SampleNoiid", "rb") as f:
        Sample = pickle.load(f)
    with open("./datasets/cifar10/LabelNoiid", "rb") as f:
        Label = pickle.load(f)
    ...
    clientDataIndex = createNoiidClientDataset()
    ...
    criterion_model = torch.nn.CrossEntropyLoss().to(device)
    
    agg = Aggregator(device)
    # model_fp = os.path.join("save/Img-10-test", "checkpoint_{}.tar".format(68))
    ...
    localParams=["instance_projector"]
    # localParams=["instance_projector"]
    print(localParams)
    clientsSavedParams={}
    for c in range(args.n_clients):
    ...
    #     {'params': res_params2, 'lr': args.resnet_lr},
    #     {'params': pro_params2, 'lr': args.project_lr},
    # ], weight_decay=args.weight_decay) for i in range(args.n_clients)]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #     testiid(epoch, agg.model)
        train(agg, epoch)
        save_model2(args, agg.model, clientsSavedParams, epoch)
