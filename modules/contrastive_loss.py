...
class InstanceLoss_NegSample(nn.Module):
    def __init__(self, batch_size, temperature, device):
        super(InstanceLoss_NegSample, self).__init__()
        self.batch_size = batch_size
        self.temperature = temperature
        self.device = device
    ...
    def forward(self, z_i, z_j, z_k):
        ...
        sim_ij_k = torch.matmul(z, z_k.T) / self.temperature
        logits = torch.cat((logits, sim_ij_k), dim=1)
        loss = self.criterion(logits, labels)
        loss /= N
