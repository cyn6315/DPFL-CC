...
from utils import plus_pretrain, plus_centeral_pretrain, covariance_loss, SimSiam, clustering, centeral_clustering_by_cosine_similarity, centeral_get_global_centroids, Aggregator, centeral_pretrain, setup_seed, pretrain, asymmetric_loss, symmetric_loss, get_global_centroids, clustering_by_cosine_similarity 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_dir', default='./save/CCFC++/Cifar10', type=str, help='path to experiment directory')
    
    parser.add_argument('--trial', type=str, default='v1', help='trial id')
    ...
    parser.add_argument('--lbd', type=float, default = 0.005, help='trade-off hyper')
    parser.add_argument('--p', type=float, default = 0., help='non-iid level')
    parser.add_argument('--lr', type=float, default = 0.0005, help='learning rate')
    
    parser.add_argument('--batch_size', type=int, default = 1500, help='batch_size')
    parser.add_argument('--num_workers', type=int, default = 6, help='num of workers to use')
    parser.add_argument('--mini_bs', type=int, default = 125, help='mini_bs')
    ...
    parser.add_argument('--n_clients', type=int, default = 40, help='n_clients')
    ...
    parser.add_argument('--eta', type=float, default = 0.04, help='eta')
    parser.add_argument('--sample_ratio', type=float, default = 0.1, help='global_lr')
    ...
    dataset_aug = Cifar10Dataset(Sample, Label, transformation)
    ...
    dataset_test = Cifar10DatasetTest(Sample, Label, transformation_test)
    ...
    dataset_train = Cifar10DatasetTrain(Sample, Label, transformation_test)
    ...
    Nets = {}
    # Nets, agg, pseudo_labels = plus_pretrain(Nets, args, Sample, Label, clientDataIndex, test_loader, ground_truth_all, n, trial_dir, device)
    ...
    for epoch in range(100):
        ...
        for batch_idx, x in enumerate(train_loader):
            ...
            for j in range(batch_num):
                ...
                loss = loss_cluster + args.lbd * loss_model + args.eta*loss_cov
        ...
        print('Round: ', epoch, 'Train Loss: %.3f' % (train_loss/(true_epoch*len(Label)/args.batch_size)))    
        agg.update()
