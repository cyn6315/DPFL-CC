    ...
    def update(self):
        ...
        for name, param in self.model.named_parameters ():
            if name in self.modelUpdate:
                ...
                # param.grad = updated + torch.div(noise, 500)
                param.grad = updated
                if normSum == 0:
...
def getModelUpdate(gradDict, netcopy, net):
    for name, param in net.named_parameters ():
        if param.requires_grad:
            gradDict[name] = netcopy[name].data.clone() - param.data.clone()
...
def train(agg, round):
    ...
    dataset_round = Cifar10Dataset(Sample[indices], Label[indices], transformation, transformation_ori, transformation_strong)
    ...
    for batch_idx, (x_list) in enumerate(dataLoader_round):
        model.train()
        ...
        print('Round: ', round, "User:", clients_sampled[batch_idx], 'Train Loss: %.3f' % (lossPerUser/(true_epoch+smooth_step*args.smooth_K)))
...
if __name__ == "__main__":
    ...
    config = yaml_config_hook("config/config_DP_FL_Cifar10.yaml")
    ...
    loadpath="save/Cifar-10-DPFL-ResNet18-r_conv8"
    model_fp = os.path.join(loadpath, "checkpoint_{}.tar".format(159))
